...
def load5foldData(obj,method): 
    ...
    if obj == "Pheme" and method==0:
        ...
        label_path = './data/pheme/Pheme_label_All.txt'
        # label_path=r'D:\xm\舆论场\谣言检查\论文代码\Data\data\pheme\Pheme_label_All.txt'
        # os.path
        # os.path.parent
        # os.path.parent.join(label_path)
        labelPath = os.path.join(label_path)
        ...
        print(df)
        # print(df.sort_values(['time']).groupby(['event']).apply(lambda x: x.iloc[0]))
        # print(df.sort_values(['time']).groupby(['event']).apply(lambda x: x.iloc[-1]))
        # df.groupby('event')
        # print(df)
